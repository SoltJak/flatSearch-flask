...
def search_results():
    ...
    print('M: Cena: ' + flatsCurrent[-1].price + ', Cena M2: ' + flatsCurrent[-1].pricePerM2)
    flatsCurrent = Flat.query.filter((Flat.searchCode == search_params['searchCode']) \
        # & (Flat.district == search_params['location'])).all()
        & (Flat.roomsNo == (search_params['roomsNoSearch']) + ' pokoje') \
        & ((Flat.size) >= float(search_params['flatSizeMin'])) \
        & ((Flat.size) <= float(search_params['flatSizeMax']))).all()
